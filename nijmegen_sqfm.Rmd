...
![WP detects a possible overflow](./images/overflow.png){height=60%}
...
The verification is $\color{blue}{\bf{OK}}$, but are we complete (?), consider:
...
Explicit `\result` value specification makes it complete:
...
Consider a faulty and lengthy implementation with 2 bugs:
...
The verification is inconclusive, the entire post condition failed!

![What is the actual reason?](./images/faulty.png){height=60%}

## Verification Examples: Split

What if we slit the specification?
...
This is enough hint to know where to look:

![Finding the root-causes](./images/split.png){height=60%}

## Verification Examples: BUG #1

What if we fix the first bug the specification?
...
## Verification Examples: Better!

Fixing BUG #1 turns the corresponding post-conditions green!

![Sequential issue resolution](./images/bug_1.png){height=60%}

## Verification Examples: Fixed

Fixing BUG #2 yields an $\color{blue}{\bf{OK}}$ verification result!

![Now we are all fine](./images/all_ok.png){height=60%}
